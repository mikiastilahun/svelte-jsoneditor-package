...
<script>import { createAutoScrollHandler } from '../../controls/createAutoScrollHandler';
import { faCheck, faCode, faWrench } from '@fortawesome/free-solid-svg-icons';
import { createDebug } from '../../../utils/debug';
import { compileJSONPointer, existsIn, getIn, immutableJSONPatch, isJSONPatchAdd, isJSONPatchReplace, parsePath } from 'immutable-json-patch';
import jsonrepair from 'jsonrepair';
import { initial, isEmpty, isEqual, last, noop, throttle, uniqueId } from 'lodash-es';
import { getContext, onDestroy, onMount, tick } from 'svelte';
import { createJump } from '../../../assets/jump.js/src/jump';
import { CONTEXT_MENU_HEIGHT, CONTEXT_MENU_WIDTH, MAX_DOCUMENT_SIZE_EXPAND_ALL, MAX_SEARCH_RESULTS, SCROLL_DURATION, SEARCH_UPDATE_THROTTLE, SIMPLE_MODAL_OPTIONS } from '../../../constants';
import { collapsePath, createDocumentState, documentStatePatch, expandPath, expandSection, expandSingleItem, expandWithCallback, getEnforceString, setEnforceString } from '../../../logic/documentState';
import { createHistory } from '../../../logic/history';
import { createNewValue, createRemoveOperations, duplicate, extract, insert, revertJSONPatchWithMoveOperations } from '../../../logic/operations';
import { createSearchAndReplaceAllOperations, createSearchAndReplaceOperations, search, searchNext, searchPrevious, updateSearchResult } from '../../../logic/search';
import { canConvert, createAfterSelection, createInsideSelection, createKeySelection, createMultiSelection, createSelectionFromOperations, createValueSelection, findRootPath, getInitialSelection, getSelectionDown, getSelectionLeft, getSelectionPaths, getSelectionRight, getSelectionUp, isEditingSelection, isMultiSelection, isSelectionInsidePath, isValueSelection, removeEditModeFromSelection, selectAll, selectionToPartialJson, updateSelectionInDocumentState } from '../../../logic/selection';
import { mapValidationErrors, validateJSON } from '../../../logic/validation';
import { activeElementIsChildOf, createNormalizationFunctions, encodeDataPath, findParentWithNodeName, getWindow, isChildOf, isChildOfNodeName, setCursorToEnd } from '../../../utils/domUtils';
import { convertValue, isLargeContent, normalizeJsonParseError, parsePartialJson, repairPartialJson } from '../../../utils/jsonUtils';
import { keyComboFromEvent } from '../../../utils/keyBindings';
import { isObject, isObjectOrArray, isUrl, stringConvert } from '../../../utils/typeUtils';
...
import { measure } from '../../../utils/timeUtils';
...
async function handleReplace(text, replacementText) {
    ...
    const { operations, newSelection } = createSearchAndReplaceOperations(json, documentState, replacementText, activeItem, parser);
...
async function handleReplaceAll(text, replacementText) {
    ...
    const { operations, newSelection } = createSearchAndReplaceAllOperations(json, documentState, text, replacementText, parser);
    handlePatch(operations, (patchedJson, patchedState) => ({
        state: { ...patchedState, selection: newSelection }
    }));
    await tick();
...
function handleRemove() {
    ...
    const removeSelection = isKeySelection(documentState.selection) || isValueSelection(documentState.selection)
        ? createMultiSelection(json, documentState.selection.anchorPath, documentState.selection.focusPath)
    ...
    if (isEmpty(documentState.selection.focusPath)) {
        ...
        onChange({ text: '', json: undefined }, { text, json }, {
...
function handleInsertBefore() {
    ...
    if (!isEmpty(selectionBefore.focusPath) &&
 ...
 */
function expandRecursive(json, documentState, path) {
...
<style src="./TreeMode.scss">.jse-tree-mode {
  flex: 1;
  display: flex;
...
.jse-tree-mode .jse-hidden-input-label {
  position: fixed;
  right: 0;
  top: 0;
  width: 0;
