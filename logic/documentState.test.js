...
import { collapsePath, createDocumentState, deletePath, documentStatePatch, expandPath, expandSection, expandWithCallback, filterPath, forEachVisibleIndex, getEnforceString, getVisibleCaretPositions, getVisiblePaths, shiftPath, shiftVisibleSections, syncKeys } from './documentState.js';
...
describe('documentState', () => {
    ...
    describe('expandWithCallback', () => {
        ...
        it('should expand a nested item of a json document', () => {
            assert.deepStrictEqual(expandWithCallback(json, state, ['array'], (path) => isEqual(path, ['array'])).expandedMap, {
                '/array': true
            });
        });
        ...
        it('should partially expand a json document', () => {
            assert.deepStrictEqual(expandWithCallback(json, state, [], (path) => path.length <= 1).expandedMap, {
                '': true,
                '/array': true,
                '/object': true
            });
        ...
        it('should expand the root of a json document', () => {
            assert.deepStrictEqual(expandWithCallback(json, state, [], (path) => path.length === 0).expandedMap, {
                '': true
            });
        });
        ...
        it('should leave already expanded nodes as is', () => {
            ...
            assert.deepStrictEqual(expandWithCallback(json, stateWithExpanded, [], () => false).expandedMap, expandedMap);
        });
    });
    ...
    it('should keep/update enforce string', () => {
        ...
        assert.strictEqual(getEnforceString(json1, documentState1.enforceStringMap, compileJSONPointer([]), JSON), false);
        const json2 = '42';
        const documentState2 = createDocumentState();
        assert.strictEqual(getEnforceString(json2, documentState2.enforceStringMap, compileJSONPointer([]), JSON), true);
    });
    describe('documentStatePatch', () => {
        ...
        it('replace: should keep enforceString state', () => {
            ...
            assert.strictEqual(getEnforceString(json, documentState.enforceStringMap, pointer, JSON), true);
            const operations = [{ op: 'replace', path: '', value: 'forty two' }];
            const res = documentStatePatch(json, documentState, operations);
            assert.deepStrictEqual(getEnforceString(res.json, res.documentState.enforceStringMap, pointer, JSON), true);
        });
        ...
        it('remove: should remove a value from an array', () => {
            ...
            assert.deepStrictEqual(res.documentState, {
                ...documentState,
                expandedMap: deleteIn(documentState.expandedMap, ['/members/2']),
                visibleSectionsMap: {
